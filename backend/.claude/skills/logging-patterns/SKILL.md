...
SPRING_PROFILES_ACTIVE=human-logs ./gradlew bootRun
```

---

## Setup for Spring Boot < 3.4 (legacy)

> Spring Boot **3.4+ has native structured logging** (`logging.structured.format.console=ecs`) —
> prefer that (see the `observability` skill). This Logback encoder is only for older versions.
