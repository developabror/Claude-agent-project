...
- Secrets via env/secret store, never baked into images or committed. `docker-compose`/`compose.yaml` brings up Postgres (+ deps) for one-command local parity.

## Definition of done
- `./gradlew build` and `docker build` succeed locally (show output); the image runs and `/actuator/health` is UP.
