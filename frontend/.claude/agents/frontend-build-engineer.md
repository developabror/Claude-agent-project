...
- **Build-once/run-anywhere**: inject runtime config (`window.__ENV__`) at container start rather than baking env into the bundle, so one image serves all environments. nginx **proxies `/api` → backend** (root stack) or → the Prism `mock-api` (frontend-only stack) — make the upstream configurable.
...
- **Own `frontend/compose.yaml`** (frontend + Prism mock of `backend/openapi.json`) and keep the **root `compose.yaml`** frontend service in sync — update both together when ports/env/upstream change (see `local-stack`).

## Definition of done
- `npm run build` and `docker build` succeed (show output).
- **`/redeploy --frontend` brings up the stack green** (UI serves, deep-link falls back, reaches the mock through `/api`) — the running system is the proof.
